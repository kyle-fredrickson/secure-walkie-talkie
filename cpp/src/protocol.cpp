...
bool protocol::verify_request(const JSON& request, const BigInt& bob_rsa_d,
    const BigInt& bob_rsa_n, const JSON& bob_contacts, JSON& retained)
{
  ...
  BigInt tod = BigInt(sess_key["tod"].get<std::string>());
  ...
  BigInt alice_dh_pub = BigInt(agree["diffie_pub_k"].get<std::string>());
  ...
  // Step 10: Verify that Alice is one of Bob's contacts through the signature.
  bool verified = false;

  for (auto& contact : bob_contacts) {
    BigInt alice_rsa_n = BigInt(contact["rsa_n"].get<std::string>());
    BigInt alice_rsa_e = BigInt(contact["rsa_e"].get<std::string>());
  ...
  // Step 11: Pass back the retained data: ToD and Alice's public Diffie-Hellman key.
  retained = {
    { "tod", tod.str_in_base(10) },
    { "alice_dh_pub", alice_dh_pub.str_in_base(10) }
  };
